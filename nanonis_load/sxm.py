...
def sxm_header(filename, extra_info=None):
    ...
    temp = [
        chnls.split("\t") for chnls in header[":DATA_INFO:"][1:-1]
    ]  # Will this handle multipass?
    header["channels"] = [
        chnls[2].replace("_", " ") + " (" + chnls[3] + ")" for chnls in temp
    ]
...
class Sxm:
    ...
    def k_to_ij(self, k: np.ndarray, round=False, two_pi=False) -> np.ndarray:
        ...
        """
        pixel_coords = (
            (k - self.fft_bottom_left_corner) / self.fft_range * self.xy_pixels
        )
    ...
    def crop_pixels_window(
        self, bottom_left_x: int, bottom_left_y, width: int, height: int
    ):
        r"""
    ...
    def set_resolution(
        ...
        if self.x_mask is not None or self.y_mask is not None:
            print(
                "Warning: x_mask or y_mask is already defined and may not match new resolution."
            )
        ...
        for channel in channels:
            ...
            for idx in directions:
                interp_image = griddata(
                    old_points, self.data[channel][idx].ravel(), new_points
                )
...
def moisan_decomposition(data: np.ndarray) -> np.ndarray:
    ...
    s_denom = (
        ...
        + 2
        * np.tile(
            np.cos(2 * np.pi * Y.reshape(-1, 1) / data.shape[0]), (1, data.shape[1])
        )
        - 4
    )
    s_denom[0, 0] = 1
...
class Plot:
    ...
    def __init__(
        ...
        y, x = np.mgrid[
            0 : x_range : (x_pixels + 1) * 1j, 0 : y_range : (y_pixels + 1) * 1j
        ]
        # x = x.T
        # y = y.T
    ...
    def add_spectra(self, spectra, labels=None, channel = "Bias calc (V)"):
        ...
        for spectrum_inst, label_inst in zip(spectra_iterator, label_iterator):
            spectrum_to_center = (
                spectrum_inst.header["x (nm)"] - self.data.header["x_center (nm)"],
                spectrum_inst.header["y (nm)"] - self.data.header["y_center (nm)"],
            )
