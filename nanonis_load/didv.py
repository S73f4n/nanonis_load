...
This defaults to channel = 'Input 2 (V)' or 'Input 2 [AVG] (V)'. 
Use double_lockin = True to average with 'Input 3 (V)'.
...
from . import sxm
from .util import copy_text_to_clipboard, get_cmap_from_digitalizer_file
...
class Spectrum:
    ...
    def get_integrated_data(self, channel, scale_factor: float = 1.0) -> pd.DataFrame:
        ...
        for i in range(i0):
            integrated_data[i] = -scipy.integrate.trapezoid(
                self.data[channel][i:i0], self.data["Bias calc (V)"][i:i0]
            )
        for i in range(i0, len(self.data)):
            integrated_data[i] = scipy.integrate.trapezoid(
                self.data[channel][i0:i], self.data["Bias calc (V)"][i0:i]
            )
        data = pd.DataFrame(
            {
                "Bias calc (V)": self.data["Bias calc (V)"],
                f"Integrated {channel}": integrated_data * scale_factor,
            }
        )
        return data

    def get_lockin_calibration_factor(
        self, lockin_channel: str = "Input 2 (V)", method: str = "derivative"
    ) -> float:
        """
        ...
        if method == "derivative":
            return np.linalg.lstsq(
                self.data[lockin_channel].to_numpy()[:, np.newaxis],
                np.gradient(self.data["Current (A)"], self.data["Bias calc (V)"]),
                rcond=None,
            )[0]
        elif method == "integral" or method == "integrate":
            return np.linalg.lstsq(
                self.get_integrated_data(channel=lockin_channel)[
                    f"Integrated {lockin_channel}"
                ].to_numpy()[:, np.newaxis],
                self.data["Current (A)"],
                rcond=None,
            )[0]
    ...
    def plot(
        ...
        **kwargs,
        ...
        if plot_on_previous:
            return dat.plot(
                x="Bias calc (V)", y=channel, label=legend_label, ax=ax, **kwargs
            )
    ...
    def get_gap_bounds(
        ...
        if mode == "fwhm" or mode == "full width half max":
            ...
            if len(positive_roots) == 0 or len(negative_roots) == 0:
                return 0, 0
            return negative_roots[np.argmin(np.abs(negative_roots))], np.amin(
                positive_roots
            )
        elif mode == "derivative":
            peaks, peak_properties = scipy.signal.find_peaks(
                np.gradient(y), prominence=prominence
            )
            dips, dip_properties = scipy.signal.find_peaks(
                -np.gradient(y), prominence=prominence
            )
            if verbose:
            ...
            return x[
                sorted(
                    [
                        dips[np.argmax(dip_properties["prominences"])],
                        peaks[np.argmax(peak_properties["prominences"])],
                    ]
                )
            ]
            ...
            if len(peaks) == 0:
                return np.array([0, 0])
            return x[
                sorted([peaks[sorted_indices[-2]], peaks[sorted_indices[-1]]])
            ]  # Indices of the two biggest peaks
        ...
        **kwargs,
        ...
        for idx, spectrum_inst in enumerate(spectra_iterator):
            ...
            if offsetY is not None:
                spec_data[channel] = offsetY + spec_data[channel]
            if logabs:
            ...
            if waterfall != 0:
                spec_data[channel] = (
                    spec_data[channel]
                    + waterfall * idx * np.sign(increment)
                    + 0.5 * (-np.sign(increment) + 1) * waterfall * (len(spectra) - 1)
                )
                if color is None:
...
class Colorplot(interactive_colorplot.Colorplot):
    ...
    def __init__(self, *spectra_list, **kwargs):
        ...
        index_label = (
            kwargs["index_label"] if ("index_label" in kwargs) else "Gate Voltage (V)"
        )
        ...
        post_transform = (
            kwargs["post_transform"] if ("post_transform" in kwargs) else None
        )
        running_index = (
            kwargs["running_index"] if ("running_index" in kwargs) else False
        )
        ...
        self.spectra_list = parse_arguments(
            *spectra_list, cache=cache, constraint=constraint
        )
        ...
        if channel is None:
            for spec in self.spectra_list:
                spec.data.rename(
                    columns={"Input 2 [AVG] (V)": "Input 2 (V)"}, inplace=True
                )
                if double_lockin:
                    spec.data.rename(
                        columns={"Input 3 [AVG] (V)": "Input 3 (V)"}, inplace=True
                    )
                    spec.data["Input 2 (V)"] = (
                        spec.data["Input 2 (V)"] + spec.data["Input 3 (V)"]
                    ) * 0.5
        ...
        if self.transform is None:
            if self.multiply is None:
                self.data = pd.concat(
                    (spec.data[self.channel] for spec in self.spectra_list), axis=1
                ).values
            else:
            ...
            if self.over_iv is not None:
                self.current = (
                    pd.concat(
                        (
                            (
                                spec.data.get("Current (A)", 0)
                                + spec.data.get("Current [AVG] (A)", 0)
                            )
                            for spec in self.spectra_list
                        ),
                        axis=1,
        ...
        if running_index:
            self.index_list = np.array(
                [
                    int(s._filename.split(".")[0].split("_")[-1])
            ...
            if index_range is None:
                if start is None:
                    ...
                    if increment is None:
                        ...
                        index_range = (
                            np.arange(len(self.spectra_list)) * increment + start
                        )  # increment must be signed
            if len(index_range) == 2:
                self.index_list = np.linspace(
                    index_range[0], index_range[1], len(self.spectra_list)
                )
            ...
            if yaxis is None:
                if gate_as_index and (start is None) and (increment is None):
                    self.index_list = np.array(
                        [spec.gate for spec in self.spectra_list]
                    )
            else:
                self.state_for_update["yaxis"] = yaxis
                if isinstance(yaxis, str):
                    self.ylist = np.array(
                        [getattr(spec, yaxis) for spec in self.spectra_list]
                    )
                elif callable(yaxis):
        ...
        self.pcolor = self.ax.pcolormesh(
            x, y, self.data, cmap=pcolor_cm, rasterized=rasterized
        )
        ...
        # Image data markers
        self.img_data_points = (
            {}
        )  # Keys should be the tuple (V_s, V_g) and values should be filenames
        self.marker_annot = self.ax.annotate(
            "",
            xy=(0, 0),
            xytext=(20, 20),
    ...
    def get_spectra_gate_range(
        self, start_gate: float, end_gate: float
    ) -> list[Spectrum]:
        spectra = []
    ...
    def load_data(self, cache=None):
        constraint = (
            self.initial_kwarg_state["constraint"]
            if ("constraint" in self.initial_kwarg_state)
            else None
        )
        cache = (
            self.initial_kwarg_state["cache"]
            if ("cache" in self.initial_kwarg_state)
            else None
        )
        self.spectra_list = parse_arguments(
            *self.arg_list, cache=cache, constraint=constraint
        )
        ...
        self.data = pd.concat(
            (spec.data[self.channel] for spec in self.spectra_list), axis=1
        ).values  # No transform, multiply, over_iv
        if self.transform is None:
            if self.multiply is None:
                self.data = pd.concat(
                    (spec.data[self.channel] for spec in self.spectra_list), axis=1
    ...
    def update(self):
        colorbar = (
            self.initial_kwarg_state["colorbar"]
            if ("colorbar" in self.initial_kwarg_state)
            else True
        )
        super(Colorplot, self).update(
    ...
    def extract_peaks(
        ...
        if (
            biases[-1] < biases[0]
        ):  # Flip along bias axis if the sweep was done backwards
        ...
        peak_widths = np.zeros(len(gate_range))
        ...
        for i in range(start_gate_ind, end_gate_ind + 1):
            ...
            spectrum = scipy.ndimage.gaussian_filter(
                spectra[i, start_bias_ind:end_bias_ind], sigma=energy_blur
            )
            ...
            try:
                ...
                if fit[0] < fit_bias.max() and fit[0] > fit_bias.min():
                    ...
                    peak_widths[i - start_gate_ind] = 2.355 * fit[2]
                else:
                    peak_energies[i - start_gate_ind] = fit_bias[np.argmax(spectrum)]
                    peak_widths[i - start_gate_ind] = 0

            except RuntimeError as e:
                print(f"Error fitting point ({i}). Falling back to {fallback}.")
                if verbose:
                    print("vervose")
                    print(e)
                if fallback == "max":
        ...
        return peak_energies, peak_widths, gate_range, fit_bias
    ...
    def get_peak_cluster(self, bias, gate):
        gate_index = min(
            range(len(self.gate)), key=lambda idx: abs(self.gate[idx] - gate)
        )
        bias_index = min(
            range(len(self.bias)), key=lambda idx: abs(self.bias[idx] - bias)
        )
        nearest_pair_index = min(
            range(len(self._peak_pairs)),
            key=lambda idx: (bias_index - self._peak_pairs[idx][0]) ** 2
            + (gate_index - self._peak_pairs[idx][1]) ** 2,
        )
        cluster_index = self._peak_cluster_labels[nearest_pair_index]
        cluster_list = [
            idx
            for idx, val in enumerate(self._peak_cluster_labels)
            if val == cluster_index
        ]
    ...
    def peak_follower(self, start_bias, start_gate, end_gate, cluster_filter=False):
        start_gate_index = min(
            range(len(self.gate)), key=lambda idx: abs(self.gate[idx] - start_gate)
        )
        end_gate_index = min(
            range(len(self.gate)), key=lambda idx: abs(self.gate[idx] - end_gate)
        )
        start_bias_index = min(
            range(len(self.bias)), key=lambda idx: abs(self.bias[idx] - start_bias)
        )
        ...
        index_into_pair = min(
            range(len(starting_pairs)),
            key=lambda idx: abs(starting_pairs[idx][0][0] - start_bias_index),
        )
        ...
        while curr_gate_index != end_gate_index:
            ...
            searched_biases = np.array(
                [pair[0] for pair in relevant_pairs if pair[1] == curr_gate_index]
            )
    ...
    def linecut(self, startPoint=None, endPoint=None, ninterp=200, axes=None):
        ...
        def on_click(event):
            if (event.xdata is not None) and (event.ydata is not None):
                if first_pt[0] is None:
                    ...
                    line[0] = ax.plot(
                        [first_pt[0], first_pt[0]],
                        [first_pt[1], first_pt[1]],
                        pickradius=pickrad,
                    )
                    event_handlers[1] = fig.canvas.mpl_connect(
                        "motion_notify_event", on_motion
                    )
                    event_handlers[2] = fig.canvas.mpl_connect(
                        "button_release_event", on_release
                    )
                else:
                    try:
                        first_dist = (event.xdata - first_pt[0]) ** 2 + (
                            event.ydata - first_pt[1]
                        ) ** 2
                        last_dist = (event.xdata - last_pt[0]) ** 2 + (
                            event.ydata - last_pt[1]
                        ) ** 2
                        distSens = 1
        ...
        if (startPoint is not None) and (endPoint is not None):
            ...
            last_pt[1] = y[nearest_idx]
            line[0] = ax.plot(
                [first_pt[0], last_pt[0]], [first_pt[1], last_pt[1]], pickradius=pickrad
            )
            build_line()
    ...
    def to_html(self, filename, library="altair"):
        ...
        if library.lower() == "altair":
            ...
            x, y = self.mesh()
            x = (
                np.repeat(x, 2, axis=0)[1:-1, :-1]
                .reshape(self.data.shape[0], 2, -1)
                .transpose((1, 0, 2))
                .reshape(2, -1)
            )
            ...
            selectorMin = alt.selection_single(
                ...
                bind=alt.binding_range(
                    min=mindidv, max=maxdidv, step=didvresolution, name="Color Minimum"
                ),
                init={"minimum": mindidv},
            )
            selectorMax = alt.selection_single(
                name="maxSelect",
                fields=["maximum"],
                bind=alt.binding_range(
                    min=mindidv, max=maxdidv, step=didvresolution, name="Color Maximum"
                ),
                init={"maximum": maxdidv},
            )
            ...
            cplot = (
                base.properties(height=500, width=300)
                .mark_rect()
                .transform_calculate(
                ...
                .interactive()
                .add_selection(selectorMin)
                .add_selection(selectorMax)
            )
            ...
            rule = (
                base.properties(height=500, width=300)
                .mark_rule()
                .transform_calculate(gate="(datum.y + datum.y2) / 2.0")
                .encode(
                    y="y:Q",
            ...
            lineplot = (
                base.properties(height=200, width=300)
                .transform_filter("datum.y == mouseSelect.y")
                .mark_line()
                .transform_calculate(centered_x=(alt.datum.x2 + alt.datum.x) / 2.0)
                .encode(
            ...
            background = (
                ...
                .transform_filter("!isValid(mouseSelect.y)")
    ...
    def bias_and_gate_in_range(self, sample_bias, gate_voltage, tolerance=100):
        ...
        return (
            (sample_bias > bias_min - bias_tolerance)
            & (sample_bias < bias_max + bias_tolerance)
            & (gate_voltage > gate_min - gate_tolerance)
            & (gate_voltage < gate_max + gate_tolerance)
        )
    ...
    def add_img_data_marker(self, filename):
        ...
        try:
            ...
            print(
                "Warning: " + filename + " does not have the gate voltage stored in it"
            )
            return
        ...
        if "multipass_biases" in header.keys():
            ...
            sample_bias = float(
                header[":BIAS:"][0]
            )  # If this throws an exception, then the header is probably fucked up
            self.img_data_points[(round(sample_bias, 5), gate_voltage)] = filename
    ...
    def add_img_data_marker_manual(self, filename, sample_bias, gate_voltage):
        ...
        """
        if (
            (sample_bias > np.amin(self.xlist))
            & (sample_bias < np.amax(self.xlist))
            & (gate_voltage > np.amin(self.ylist))
            & (gate_voltage < np.amax(self.ylist))
        ):
    ...
    # Fixed using dicts
    def auto_add_img_data_markers(
        self, basename=None, start=0, end=99999, index_list=None
    ):
        """
                ...
                if basename is None:
                    ...
                    if (match is not None) and (start <= int(match.group(1)) <= end):
                        if index_list is not None:
                            if (
                                int(match.group(1)) not in index_list
                            ):  # Should check if index_list is a list, tuple, set, etc...
                                continue
                        self.add_img_data_marker(filename)
    ...
    def plot_img_data_markers(self, s=100, color=(0, 0, 0, 1), zorder=1000, **kwargs):
        ...
        self.img_data_scatter = self.ax.scatter(
            sample_biases, gate_voltages, s=s, color=color, zorder=zorder, **kwargs
        )
        self.ax.zorder = 100
    ...
    def update_annotation(self, ind):
        ...
        text = (
            ...
            + "\nVs = "
            + str(list(self.img_data_points.keys())[index][0] * 1000)
            + " mV"
            + "\nVg = "
            + str(list(self.img_data_points.keys())[index][1])
            + " V"
        )
    ...
    def on_click(self, event):
        ...
        if event.inaxes == self.ax:
            ...
            if cont:
                ...
                try:
                    multipass_index = np.argmin(
                        np.abs(image_sxm.header["multipass_biases"] - event.xdata)
                    )
                    data = image_sxm.process_data(
                        image_sxm.data["Z (m)"][multipass_index],
                        process="subtract plane",
                    )
                except KeyError:
                    data = image_sxm.process_data(
                        image_sxm.data["Z (m)"][0], process="subtract plane"
                    )
                ...
                ax[1].imshow(
                    ...
                    origin="lower",
                    extent=(
                        -np.pi / (x_range / x_pixels),
                        np.pi / (x_range / x_pixels),
                        -np.pi / (y_range / y_pixels),
                        np.pi / (y_range / y_pixels),
                    ),
                )
                fig.canvas.draw()
...
def batch_load(
    basename, file_range=None, attribute_list=None, cache=None, constraint=None
):
    ...
    if (".h5" in basename) or (".hdf5" in basename):
        ...
        return HDF5Tospecs(
            basename, cachedNames=cachedNames, returnNames=True, constraint=constraint
        )
    else:
        ...
        try:
            with h5py.File(filename, "a") as f:
                if "data" not in f.keys():
                    f.create_group(
                        "data"
                    )  # Unfortunately, track_order not available in older versions of h5py
                for spec in spectrumList:
                    ...
                    f["data"][spec._filename].attrs["channels"] = "||".join(
                        spec.data.columns
                    )
                size = len(f["data"])
                f.attrs["size"] = size
            ...
            with h5py.File(filename, "r") as f:
                for name, dataset in f["data"].items():
                    ...
                    spec.data = pd.DataFrame(
                        dataset[()], columns=dataset.attrs["channels"].split("||")
                    )
                    spec._filename = name
                    spec._fix_header()
...
def datFilesToHDF5(file_range=None):
    ...
    for file in allFiles:
        ...
        if match is not None:
            ...
            if (
                basename != "Bias-Spectroscopy"
            ):  # Will still accept 'Bias-Spectroscopy_'
...
class TransformColorplot(interactive_colorplot.Colorplot):
    r"""
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.func = args[0]
        self._cplots = args[1:]
        self.xlist = self._cplots[
            0
        ].bias  # Assumes bias for all self._cplots are the same
        self.load_data()
        pseudocoordX, pseudocoordY = self.mesh()
        self.fig = plt.figure()
        self.ax = self.fig.add_subplot(111)
        self.pcolor = self.ax.pcolormesh(
            pseudocoordX, pseudocoordY, self.data, cmap=pcolor_cm, rasterized=rasterized
        )
        self.original_cmap = self.pcolor.cmap
        self.colorbar = self.fig.colorbar(self.pcolor, ax=self.ax)
        self.ax.set_xlabel("Sample Bias (V)")
...
class MultiColorplot:
    ...
    def add_data(self, *args, **kwargs):
        ...
        if self.count < self.max:
            new_colorplot = quick_colorplot(
                *args, axes=self.axes[len(self.colorplots)], **kwargs
            )
            self.colorplots.append(new_colorplot)
            self.drag_bars.append(
                new_colorplot.drag_bar(
                    direction=self.direction,
                    axes=self.drag_ax,
                    color=self._color_cycle[self.count],
                )
            )
            self.count += 1
        else:
...
def std_ping_remove(
    spec, n
):  # Removes pings from Input 2 [...] (V), if average over 3 sweeps or more
    data = pd.DataFrame()
    cnt = 0
    ...
    std = data.std(
        axis=1
    )  # Maybe use interquartile range instead of standard deviation
    median = data.median(axis=1)
    data[np.abs(data.sub(median, axis=0)).gt(n * std, axis=0)] = np.nan
    spec.data["Input 2 (V)"] = data.mean(axis=1)


class LineCutGateSweep:
    """
    Load and plot data from a list of .dat files where both the gate voltage
    and the tip position are sweeped.
    ...
    num_gates : int
        The number of gate voltages in the data
    gates : list[float]
        List of gate voltages
    num_pos : int
        Number of tip positions in the linecut
    data : ndarray
        3-dimensional array containing the data with shape (num_pos, num_gates, -1)
    distance : float
        The physical length of the linecut in nm
    bias_lower : float
        The min
    """
    ...
    def __init__(
        ...
        # Number of positions is number of spectra / num_gates
        self.num_pos = int(len(self.spectra) / self.num_gates)
        ...
        if normalize == "integral":
            for i in range(self.num_pos):
                for j in range(self.num_gates):
                    self.data[i, j, :] = self.data[i, j, :] / np.sum(
                        self.data[i, j, :], axis=-1
                    )
        elif normalize == "max":
            for i in range(self.num_pos):
                for j in range(self.num_gates):
                    self.data[i, j, :] = self.data[i, j, :] / np.amax(
                        self.data[i, j, :], axis=-1
                    )
        ...
        if slider_axis == "gate":
            self.slider = matplotlib.widgets.Slider(
                ...
                valmin=np.amin(self.gates),
                valmax=np.amax(self.gates),
                valinit=np.amin(self.gates),
            )
            self.im = self.ax.imshow(
                self.data[:, 0, :].T,
                origin="lower",
                extent=(0, self.distance, self.bias_lower, self.bias_upper),
                aspect="auto",
                vmin=0,
            ...
            def update(gate: float):
                ind = np.argmin(np.abs(self.gates - gate))
                self.im.set_data(self.data[:, ind, :].T)
                self.im.set_clim(0, np.amax(self.data[:, ind, :]))
                self.fig.canvas.draw()
            ...
            def update(d: float):
                ind = int(d / self.distance * self.num_pos)
                self.im.set_data(self.data[ind, :, :].T)
                self.im.set_clim(0, np.amax(self.data[ind, :, :]))
                self.fig.canvas.draw()
    ...
    def bias_lower(self):
        """
        Returns the lower bias of the sweep regardless of the direction the
        bias was sweeped in
        """
        sweep_start = float(self.spectra[0].header["Sweep Start"]) * 1000
        sweep_end = float(self.spectra[0].header["Sweep End"]) * 1000

        return min(sweep_start, sweep_end)

    @property
    def bias_upper(self):
        """
        Returns the upper bias of the sweep regardless of the direction the
        bias was sweeped in
        """
        sweep_start = float(self.spectra[0].header["Sweep Start"]) * 1000
        sweep_end = float(self.spectra[0].header["Sweep End"]) * 1000
    ...
    def create_movie(
        ...
        ----------
        filename : str
            The filename to save the movie as. Default is movie.mp4
        interval : int
            Delay between frames in milliseconds. Default is 50
        blit : bool
            Whether blitting is used to optimize drawing. Default is True
        constant_vmax: float
        ...
        if self.slider_axis == "gate":
            ...
            for i in range(self.num_gates):
                im = ax.imshow(
                    ...
                    aspect="auto",
                    vmin=0,
                    vmax=np.amax(self.data[:, i, :]) if vmax is None else vmax,
                    cmap="RdYlBu_r",
                )
                text = plt.text(
                    0.5,
        ...
        ani = matplotlib.animation.ArtistAnimation(
            fig, frames, interval=interval, blit=blit
        )
        ani.save(filename)
...
class _QueryTransformer(ast.NodeTransformer):
    ...
    def visit_Name(self, node):
        return ast.Attribute(
            value=ast.Name(id="spec", ctx=ast.Load()), attr=node.id, ctx=node.ctx
        )
...
def fixed_bias_plot(
    ...
    bias_list = spectra_list[0].data[
        "Bias calc (V)"
    ]  # Assumes all spectra have same bias list
    try:
        left_bias = bias[0]
        right_bias = bias[1]
        left_bias_index = min(
            range(len(bias_list)), key=lambda bidx: abs(bias_list[bidx] - left_bias)
        )
        right_bias_index = min(
            range(len(bias_list)), key=lambda bidx: abs(bias_list[bidx] - right_bias)
        )
        ...
        bias_index = min(
            range(len(bias_list)), key=lambda bidx: abs(bias_list[bidx] - bias)
        )
        nbiases = 1
    ...
    if normalize:
        ...
        gate_index_between = [
            enum_g[0]
            for enum_g in enumerate(gate_cp)
            if normalize[0] <= enum_g[1] <= normalize[1]
        ]
...
def fixed_gate_plot(
    ...
    try:
        ...
        if left_gate < right_gate:
            spectra = query(
                spectra_list, str(left_gate) + "<= gate <= " + str(right_gate)
            )
        else:
            spectra = query(
                spectra_list, str(right_gate) + "<= gate <= " + str(left_gate)
            )
    except TypeError:
...
class LandauFan:
    ...
    def __init__(self, filename, cache=None, fast=False):
        ...
        with open(filename, "r") as f:
            ...
            for fline in f:
                ...
                if (
                    (cache is not None)
                    and f_idx < len(cache._basenames)
                    and (field_line[3:] == cache._basenames[f_idx])
                ):
                    if fast is False:
                        self.spectra_list.append(
                            parse_arguments(
                                *field_line[3:], cache=cache.spectra_list[f_idx]
                            )
                        )
                    else:
    ...
    def get_index_for_B(self, B):
        nearest_B_index, nearest_B = min(
            enumerate(self.magnet), key=lambda x: abs(x[1] - B)
        )
        ...
        for idx in range(self.num_fields):
            ...
            if width is None:
                if self.num_fields == 1:
                    ...
                    try:
                        smaller_fvalue = max(
                            [val for val in self.magnet if val < field_value]
                        )
                ...
                if center:
                    even_bound = min(
                        [upper_bound - field_value, field_value - lower_bound]
                    )
            ...
            parent = [self, idx]
            fbplot = fixed_bias_plot(
                self.spectra_list[idx],
                bias,
                lower_bound,
                upper_bound,
                axes=self.ax,
                cmap=cmap,
                shift_gate=self._shift_gate[idx],
                flip_bias=flip_bias,
                normalize=normalize,
                rasterized=rasterized,
                parent=parent,
        ...
        def on_click(event):
            if (event.xdata is not None) and (event.ydata is not None):
                if first_pt[0] is None:
                    ...
                    line[0] = ax.plot(
                        [first_pt[0], first_pt[0]], [first_pt[1], first_pt[1]]
                    )
                    event_handlers[1] = fig.canvas.mpl_connect(
                        "motion_notify_event", on_motion
                    )
    ...
    def waterfall(self, vertical_shift):
        ...
        for nth in range(self.num_fields):
            ...
            bias_list = self.spectra_list[idx][0].data[
                "Bias calc (V)"
            ]  # Assumes all spectra have same bias list
            bias_index = min(
                range(len(bias_list)), key=lambda bidx: abs(bias_list[bidx] - self.bias)
            )
...
class Butterfly:
        ...
        for idx in range(self.l_fan.num_fields):
            ...
            if width is None:
                if self.l_fan.num_fields == 1:
                    ...
                    try:
                        smaller_fvalue = max(
                            [val for val in self.l_fan.magnet if val < field_value]
                        )
                        lower_bound = (smaller_fvalue + field_value) * 0.5
                    except ValueError:
                        larger_fvalue = min(
            ...
            if scale is None:
                ...
                scale_val = scale[idx]
            fgplot = fixed_gate_plot(
                self.l_fan.spectra_list[idx],
                gate,
                lower_bound,
                upper_bound,
                axes=self._evb_ax,
    ...
    def get_index_for_B(self, B):
        nearest_B_index, nearest_B = min(
            enumerate(self.l_fan.magnet), key=lambda x: abs(x[1] - B)
        )
        return nearest_B_index
    ...
    def waterfall(self, vertical_shift):
        ...
        for nth in range(self.l_fan.num_fields):
            ...
            data = (
                np.array(spectra[0].data["Input 2 (V)"]) / (chigh - clow)
                - clow
                + nth_field * vertical_shift
            )

            wat_line = self._waterfall_ax.plot(bias, data)


def quick_landau_fan(
    filename,
    bias=0,
    cmap=None,
    center=False,
    ...
    fan = LandauFan(filename, cache=cache, fast=fast)
    fan.plot(
        bias,
        cmap=cmap,
        center=center,
